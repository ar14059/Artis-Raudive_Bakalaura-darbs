...
def get_certain_article_data(rel_uri, snd_level_arr = second_level):
            ...
            if(key == "links"):
                ...
                if(tmp_rel_type not in main_data_types):
                    for rel_key, rel_val in val.items():
                        if(tmp_rel_type in snd_level_arr and rel_key in snd_level_arr[tmp_rel_type]):
                            snd_level_rel_uri = rel_val["links"]["related"]

                            final_key = dbp_uri(rel_key)
                            tmp_list.update({final_key:[schema_val(snd_level_rel_uri)]})

                            tmp_list_arr_2nd_level = get_certain_article_data(snd_level_rel_uri)
...
#                         "animeStaff": "people", "animeProductions":"producer"}  
    ...
    certaian_article_data = get_certain_article_data(uri)
