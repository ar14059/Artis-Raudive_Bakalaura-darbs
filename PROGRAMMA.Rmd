...
# ex_data = get_kitsu_data_by_uri(ex_uri, headers)  
# print(json.dumps(ex_data))
...
second_level = {"castings":"person", "characters": "character", "castings": "person",
                "staff": "person", "staff": "person", "production": "company"}
def get_relationships_data(rel_uri, rel_type = "", snd_level = second_level):
    tmp_data = get_kitsu_data_by_uri(rel_uri, headers)
...
#     first_level = ["genres", "categories", "reviews", 
#                    "mediaRelationships", "episodes", ]


#     questionable_level = ["quotes"]
#         quotes (API nederīgs)
    tmp_list_arr = []
    tmp_list_main = {}
    tmp_list_main.update({"@id":rel_uri})
    consists_of = dbp_uri("consistsOf")
    tmp_list_main.update({consists_of:[]})
    for data_el in data:
        final_key = dbp_uri("has"+(rel_type).capitalize())
                ...
                if(val_type(val)=="list"):
                    ...
                    for attr_key, attr_val in val.items():
                        if(attr_key not in dont_need):
                            final_key = dbp_uri(attr_key, rel_type)
                            tmp_list.update({final_key:[schema_val(attr_val)]})
            elif(key == "relationships"): 
                if(rel_type in snd_level and snd_level[rel_type] != ""): 
                    
                    snd_level_rel = snd_level[rel_type]
                    snd_level_rel_val = val[snd_level_rel]["links"]["related"]
                
                    final_key = dbp_uri(snd_level_rel)
                    tmp_list.update({final_key:[schema_val(snd_level_rel_val)]})

                    tmp_list_arr_1st_level = get_relationships_data(snd_level_rel_val, snd_level_rel)
                    
                    for el_1st in tmp_list_arr_1st_level:
                        tmp_list_arr.append(el_1st)
                    
            else:
                final_key = dbp_uri(key, rel_type)
                tmp_list.update({final_key:[schema_val(val)]})
        tmp_list_arr.append(tmp_list)
    tmp_list_arr.insert(0,tmp_list_main)
...
def final_data(json_data, rec_tmp_data_start, last_key, curr_id_pref = ""): 
    rec_tmp_data = rec_tmp_data_start

    rel_exceptions = [ "installments", "mappings", "quotes", "streamingLinks", 
                       "animeProductions", "productions", "staff"]   
...
#         final_key = "uri:" + key
        ...
        if(val_type(val)!="dict"):
            if(val_type(val)=="list"):
                if(val==[]):
                    rec_tmp_data[pointer].update({final_key:"Null"})
                else:
                    tmp_arr = []
                    for arr_el in val:
                        tmp_arr.append(schema_val(arr_el))
                    rec_tmp_data[pointer].update({final_key:tmp_arr})
            else:
            ...
            or (represents_type(next(iter(val)))=="lang_ext")):
                ...
                if(key == "relationships"):
                    for rel_key, rel_val in val.items():
                        if(rel_key not in rel_exceptions):
                            final_key = dbp_uri(rel_key)
                            rel_uri = rel_val["links"]["related"]
                            rec_tmp_data[pointer].update({final_key:[schema_val(rel_uri)]})

                            rel_arr = get_relationships_data(rel_uri, rel_key)
                            for el in rel_arr:
                                rec_tmp_data.append(el)
                else:
                    ...
                    rec_tmp_data[pointer2].update(final_data(val, rec_tmp_data, last_key, key+"_"))
    if(key != last_key):
        return {}
...
def get_one_kitsu_data(kitsu_id):
    rec_tmp_data = []
    
    uri = "https://kitsu.io/api/edge/anime/" + kitsu_id
    kitsu_json_data = get_kitsu_data_by_uri(uri, headers)
    ...
    final_data(json_data_rest, rec_tmp_data, last_key)
    
    return rec_tmp_data
    
def get_ALL_kitsu_data():
    kitsu_BIG_DATA = []
    for i in range(3,4):
        k = get_one_kitsu_data(str(i))
        kitsu_BIG_DATA.append(k)
#         print(json.dumps(kitsu_BIG_DATA[0]))
    return kitsu_BIG_DATA
        

    

k_data_all = get_ALL_kitsu_data()
k_data = get_ALL_kitsu_data()[0]

# print(json.dumps(k_data_all))
print(json.dumps(k_data))

# qw = get_relationships_data("https://kitsu.io/api/edge/anime/3/castings", "castings")
# qw2 = get_relationships_data("https://kitsu.io/api/edge/castings/180/person", "person")

# print(json.dumps(qw))
# print(json.dumps(qw2))

####### SVARĪGS IZVADS, NEDZĒST #########
# get_one_kitsu_data("10057")
...
g_kitsu_data = g_kitsu.parse(data=bytes(json.dumps(k_data_all[0]),encoding='utf8'), format='json-ld')
