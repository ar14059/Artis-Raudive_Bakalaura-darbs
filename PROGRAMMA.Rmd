...
# get_max_data_ex_uri = "https://kitsu.io/api/edge/anime/10/episodes"
# get_max_data_ex = get_max_data(get_max_data_ex_uri)
# print(json.dumps(get_max_data_ex))
...
def get_certain_article_data(uri, main_data_types, including_rel, rel_second_level):
    ...
    if(data != []):
        ...
        for data_el in data:
            ...
            if(tmp_rel_type not in main_data_types):
                tmp_list_main[consists_of].insert(0,schema_val(data_el["links"]["self"]))    
...
#                                 tmp_list.update({final_key:schema_val(attr_val)})
                            ...
                            if(attr_key not in dont_need):
                                if(attr_val != "" and attr_val != None and attr_val != []):
                    ...
                    if(tmp_rel_type not in main_data_types):
                        for rel_key, rel_val in val.items():
                            if(tmp_rel_type in rel_second_level and rel_key in rel_second_level[tmp_rel_type]):
                                ...
                                tmp_list_arr_2nd_level = check_empty_val(get_certain_article_data(snd_level_rel_uri, main_data_types, including_rel, rel_second_level))
...
# ex_uri = "https://kitsu.io/api/edge/anime/10/animeProductions"
...
vvv = {"er":{"ty":"as"}}
# dict_ex = {kkk:vvv}
chained = get_chained_dict_data(vvv,kkk)
...
# tmp_chained = check_empty_val(chained)
# print(tmp_chained)
```
